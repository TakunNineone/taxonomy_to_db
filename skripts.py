...
BEGIN
 ...
 FOREACH subarr IN ARRAY array1
 loop
 if subarr = ANY (array2)
 then 
 return 1;
 end if;
 end loop;
...
create index va_periods_v  on va_periods (version);
...
create index va_periods_entity  on va_periods (entity);
...
create index va_periods_parentrole  on va_periods (parentrole);
...
create index va_periods_rinok  on va_periods (rinok);
